    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Tipo de proveedor <span className="text-rose-500">*</span></label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {(Object.keys(PROVIDER_INFO) as Array<keyof typeof PROVIDER_INFO>).map(t => (
                <label key={t} className={`flex items-start gap-2 px-3 py-2 border rounded-lg cursor-pointer ${providerType === t ? 'border-brand-500 bg-brand-50' : 'border-slate-200 hover:bg-slate-50'}`}>
                  <input type="radio" name="provider_type" value={t} checked={providerType === t} onChange={() => handleProviderChange(t)} className="mt-1" />
                  <div className="text-sm">
                    <div className="font-medium text-slate-900">{PROVIDER_INFO[t].label}</div>
                    <code className="text-xs text-slate-500">{t}</code>
                  </div>
                </label>
              ))}
            </div>
